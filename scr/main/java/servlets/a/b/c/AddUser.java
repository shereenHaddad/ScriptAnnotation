		...
		try {
		  ...
		  //Connection con = DriverManager.getConnection("jdbc:mysql://"+host+"/myasapp", "adminTKnvAxx", "6kcWL-WhALLs");
			...
			if(count ==0 ){
				...
				System.out.println("insert into users (name , password ) values '"+request.getParameter("userName").toString()+"' and password like '"+request.getParameter("password").toString()+"'");
				stmt.executeUpdate("insert into users (name , password ) values ('"+request.getParameter("userName").toString()+"' ,'"+request.getParameter("password").toString()+"')");
